...
export default function ExactSplitForm({
  members,
  ...
  // Calculate total exact splits entered
  const splitSum = selectedParticipants.reduce((sum, id) => {
    const val = parseFloat(splits[id]) || 0;
    return sum + val;
  }, 0);

  const diff = targetVal - splitSum;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="p-3 bg-slate-900 border border-slate-800 rounded-xl flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-xs font-semibold text-slate-300">
            Split Total: <span className="font-bold text-white">${splitSum.toFixed(2)}</span> / Target: <span className="font-bold text-white">${targetVal.toFixed(2)}</span>
          </span>
        </div>
        <div>
          {isMatched ? (
      ...
      <div className="space-y-2">
        <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">Participant Amounts</span>
        <div className="space-y-2">
          {members.map((member) => {
